...
Publishing evidence is a separate decision. An operator who wants a sourced evidence row writes a bundle by hand and applies it with the nonproduction `npm run db:upsert` command described in [database.md](database.md). The bundle still needs its own stance, reliability, and recorder. A KEV row is not permission to fabricate a forecast or to mark an event resolved.
...
- No evidence publication, probability, or event resolution
