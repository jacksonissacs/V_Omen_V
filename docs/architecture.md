...
| Unattended ingest | A manual one-source review queue exists ([source-intake.md](source-intake.md)). It does not publish evidence or run on a schedule. |
...
2. **Ingest** — `npm run intake` stages the CISA Known Exploited Vulnerabilities catalog in a local review queue ([source-intake.md](source-intake.md)). It does not publish evidence or propose a probability. A worker that does either is still later.
