...
/** One captured source version. A calendar date is not a publication timestamp. */
...
const CLOCK_TIME = /^\d{4}-\d{2}-\d{2}T/
...
export function assertSourceClock(value: string | null, label: string): string | null {
  if (value === null) return null
  if (!CLOCK_TIME.test(value)) {
    throw new PublicationValidationError(
      `${label} must be a clock time. A calendar date is not a publication timestamp.`,
    )
  }
  return value
}

export function sourceCaptureEvidenceId(intakeVersionId: string): string {
  const id = `ev-${intakeVersionId}`
  if (!ID.test(id)) {
    throw new PublicationValidationError(`Cannot map intake version ${intakeVersionId} to an evidence id.`)
  }
  return id
}

/**
 * Builds an evidence-only bundle from a captured source version.
 * `sourcePublishedDate` is not copied into `sourcePublishedAt`.
 * No observation is added.
 */
export function evidenceFromSourceCapture(
  eventId: string,
  capture: SourceCaptureRecord,
  completion: { stance: ReviewStance; reliability: number; recordedBy: string },
): EventBundle {
  if (capture.provenance !== "sourced") {
    throw new PublicationValidationError("Imported source captures stay sourced. They are not demo records.")
  }
  const sourceName = SOURCE_NAMES[capture.sourceId]
  if (!sourceName) throw new PublicationValidationError(`Unknown intake source ${capture.sourceId}.`)
  const recordedBy = completion.recordedBy.trim()
  if (!recordedBy) throw new PublicationValidationError("Reviewer is required before a source capture can be published.")
  return {
    eventId,
    observations: [],
    evidence: [
      {
        id: sourceCaptureEvidenceId(capture.intakeVersionId),
        sourceName,
        sourceUrl: capture.canonicalUrl,
        sourcePublishedAt: assertSourceClock(capture.sourcePublishedAt, "sourcePublishedAt"),
        firstObservedAt: capture.firstFetchedAt,
        summary: capture.excerpt,
        stance: completion.stance,
