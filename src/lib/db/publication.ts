...
import { databaseBigintText } from "./bigint-id"
...
import { getSourceReviewItem, type SourceReviewItem } from "./source-review"
...
function rowToOperation(row: OperationRow): PublicationOperationRecord {
  return {
    ...
    checkpointId: row.checkpoint_id === null ? null : databaseBigintText(row.checkpoint_id),
...
async function withPublicationLock<T>(client: ClientBase, key: string, body: () => Promise<T>): Promise<T> {
  await client.query("SELECT pg_advisory_lock(hashtextextended($1, 0))", [`publication:${key}`])
  try {
    return await body()
  } finally {
    await client.query("SELECT pg_advisory_unlock(hashtextextended($1, 0))", [`publication:${key}`])
  }
}
...
export async function publishEventBundle(
  ...
  return withPublicationLock(client, options.idempotencyKey, async () => {
...
export async function retryPublicationOperation(
  ...
  return withPublicationLock(client, idempotencyKey, async () => {
    await client.query("BEGIN")
  ...
  if (item.payload.kind === "source_capture") {
    if (!item.reviewStance || item.reviewReliability == null || !item.reviewedBy) {
      throw new PublicationValidationError(
        "Incomplete source capture cannot be published. Record stance and reliability at review.",
      )
    }
    return evidenceFromSourceCapture(item.eventId, item.payload.capture, {
      stance: item.reviewStance,
      reliability: item.reviewReliability,
      recordedBy: item.reviewedBy,
    })
  }
  return buildBundleFromReviewPayload(client, item.eventId, item.payload)
}
...
export async function publishApprovedReviewItem(
  ...
  await client.query("SELECT pg_advisory_lock(hashtextextended($1, 0))", [`review:${item.id}`])
  try {
    const current = await getSourceReviewItem(client, item.id)
    if (!current) throw new Error(`Review item ${item.id} was not found.`)
    if (current.status !== "approved") {
      throw new Error(
        `Review item ${item.id} must be approved before publication (currently ${current.status}).`,
      )
    }
    const bundle = await bundleForApprovedItem(client, current)
    return await publishEventBundle(client, bundle, { idempotencyKey, sourceReviewItemId: current.id })
  } finally {
    await client.query("SELECT pg_advisory_unlock(hashtextextended($1, 0))", [`review:${item.id}`])
  }
}

export async function verifyCheckpointDigest(client: ClientBase, checkpointId: string): Promise<string> {
