...
const USAGE = `Usage: omen-operator <command> [options]
  ...
  review show <id>
  review approve <id> --by <operator> [--note <text>]
...
async function main() {
  ...
  try {
    ...
    if (domain === "review" && action === "approve") {
      ...
      const item = await approveSourceReviewItem(client, id, values.by, values.note)
      console.log(JSON.stringify(item, null, 2))
      return
    }
    ...
    if (domain === "checkpoint" && action === "verify") {
      const checkpointId = values.id ? Number(values.id) : Number(rest[0])
      if (!Number.isFinite(checkpointId)) throw new Error("checkpoint verify needs --id <number>")
